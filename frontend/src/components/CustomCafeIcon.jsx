// ...existing code...
export function getGeotagIcon(theme, userPinSvg) {
  // For MapLibre, just return the SVG data URL for use in <img src={...} />
  return userPinSvg;
}

export function getIconSvg(color) {
  // Return a data URL for the SVG icon
...
export function getCustomIcon(category, theme) {
  function getCategoryColor(category) {
    if (category === "thirdwave")
      return theme.palette.accent?.main || theme.palette.secondary.main;
    if (category === "speciality") return theme.palette.primary.main;
    if (category === "roaster") return theme.palette.error.main;
    return theme.palette.primary.main;
  }
  const color = getCategoryColor(category);
  return getIconSvg(color);
}
