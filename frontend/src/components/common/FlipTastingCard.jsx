...
const FlipTastingCard = ({ tasting, isFlipped = false, onFlip, anyFlipped = false }) => {
  ...
  if (!isFlipped) {
    return (
      <Box sx={{ width: CARD_W, height: CARD_H, cursor: 'pointer', opacity: anyFlipped ? 0.25 : 1, transition: 'opacity 0.3s', pointerEvents: anyFlipped ? 'none' : 'auto', overflow: 'visible', borderRadius: `${theme.shape.borderRadius * 3}px`, boxShadow: theme.palette.mode === 'dark' ? '6px 10px 18px rgba(158,196,235,0.25)' : '6px 10px 18px rgba(10,31,51,0.35)' }} onClick={handleFlip}>
        <StyledCard elevation={0}>
          ...
          <RowAuthor>
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', justifyContent: 'center', flex: 1, px: 1.5 }}>
              <Typography sx={{ fontWeight: 700, fontSize: '16px', whiteSpace: 'nowrap', color: theme.palette.text.secondary }}>
                {new Date(tasting.createdAt || tasting.date).toLocaleDateString()}
              </Typography>
              <Typography textTransform="uppercase" sx={{ fontWeight: 400, fontSize: '16px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
